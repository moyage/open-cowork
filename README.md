...
v0.3.11 起，open-cowork 默认把当前协作状态收敛到一组小文件，例如 `.governance/AGENTS.md`、`.governance/agent-entry.md`、`.governance/agent-playbook.md`、`.governance/state.yaml`、`.governance/current-state.md`、`.governance/evidence.yaml`、`.governance/ledger.yaml` 和 `.governance/rules.yaml`。旧版本生成的大量历史目录只作为迁移来源和兼容对象存在。
...
Agent 会在内部运行项目接手入口、读取项目事实、确认要接续的 round，然后继续当前步骤。自然语言只是请求 Agent 使用项目里的接手规则；可靠触发点是项目内的确定性入口，不是靠关键词猜测，也不是要求平台里必须存在一个名叫 `open-cowork` 的 Skill。open-cowork 的应用对象是项目，不是某个单独 Agent。

## 典型使用场景

### 个人域单一 Agent 系统

一个人只使用 Codex、Claude Code 或其他单一 AI Coding 环境时，open-cowork 主要解决“长任务不断线”的问题。Agent 把本轮需求、范围、执行证据、验证结果和下一步接续状态写入项目里的当前治理文件，即使会话压缩、重开窗口或隔天继续，也可以从项目事实恢复，而不是靠聊天记录回忆。
...
如果某个 Agent 环境支持自定义 Skill，可以把这份文件注册进去；如果不支持，Agent 直接把它当成项目内接手说明读取即可。若 Agent 回复“找不到 open-cowork skill”，正确处理方式是读取 `.governance/agent-entry.md` 和当前状态文件，而不是停止接手。
...
- 当前状态文件：用少量可读、可验证、可轮转的文件承载当前事实，避免每轮工作生成大量目录。
...
- 完整实现原则：已确认的需求、范围、任务完成定义和验收标准默认要求完整实现；未经人明确批准，Agent 不得自行降级为最小实现、部分实现、延期实现或后续再补。未经批准的降级事实不能作为执行完成、证据成立、验证通过、review approve、closeout 或 archive 的依据。
...
旧版本项目可能仍有大量历史治理目录。v0.3.11 对这类项目的默认处理方式是：Agent 在安装、初始化或 setup/onboard 时自动检测旧文件，生成当前状态，把旧历史迁移到 cold history，并运行验证。人不需要额外理解或执行迁移命令。只有清理 cold history 或卸载治理文件这类破坏性动作，才需要显式确认和 receipt。
...
1. 安装、初始化、setup 或 onboard 入口自动识别旧版治理目录和协议版本。
2. 自动创建或升级 current-state 文件，把旧历史迁移到 cold history，并写入 migration receipt。
3. 自动运行 verify，确认当前状态文件、receipt 和旧版状态一致；失败时停止后续 setup/onboard。
4. 如需清理 cold history，再做 dry-run 并经人确认后执行 prune。
...
- `docs/index.md`：文档地图，说明普通读者、Agent、规格读者和历史追溯者各看哪里。
