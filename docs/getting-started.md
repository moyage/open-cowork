...
`open-cowork` 从 v0.2.5 开始把 Agent-first 作为默认使用方式；v0.2.6 起默认先生成 adoption plan；v0.2.7 起优先建立人的控制基线；v0.2.8 起把 Step 5 execution gate 接入默认推进流；v0.2.9 起进一步闭合 Step 8 review gate 和 Step 9 archive gate；v0.3.0 起把标准 Step 1-9、人类可读报告、gate 状态和归档收束审计链统一到默认协作体验；v0.3.1 起进一步确保新 change 从 Step 1 可见接入，补齐 intent / participants / change status / review runtime evidence / revise loop 等实践中的缺口。人不应该先去记命令，而应该先表达意图：
...
v0.3.1 推荐 Agent 在进入执行前先完成 Step 1-5 的真实确认链，并在 report/status 中展示 `gate_type`、`gate_state`、`approval_state`。新 change 默认处于 Step 1；`change prepare` 只代表材料准备，不代表 Step 1-5 已完成。在 review/archive 前继续显式记录 Step 8 和 Step 9 approval。它们仍然是 Agent 的内部工具；人看到的应该是“谁负责、做什么、是否确认、证据在哪里、下一步能否推进”。
