...
`open-cowork` 从 v0.2.5 开始把 Agent-first 作为默认使用方式；v0.2.6 起默认先生成 adoption plan；v0.2.7 起优先建立人的控制基线；v0.2.8 起把 step report 和 human gate approval 接入默认推进流。人不应该先去记命令，而应该先表达意图：
...
v0.2.8 推荐 Agent 在进入执行前补齐参与者矩阵、意图确认、当前步骤报告和 Step 5 approval。它们仍然是 Agent 的内部工具；人看到的应该是“谁负责、做什么、是否确认、下一步能否推进”。
