...
v0.3.5 起，已实施项目会生成 `.governance/agent-entry.md` 和 `.governance/index/active-changes.yaml`。任意 Agent 接手时必须先做项目级 activation；如果同一项目存在多个并行需求，Agent 必须展示 active change 列表并让人用自然语言选择要接续的需求，不得从聊天历史猜测。
