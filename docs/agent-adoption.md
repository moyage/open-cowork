...
3. 初始化或升级 v0.3.11 项目协作状态；若目标项目已有旧版大量治理目录，由安装入口自动迁移并验证。
...
v0.3.11 起，新项目默认不创建旧版大量治理目录。Agent 应围绕 `.governance/state.yaml`、`.governance/current-state.md`、`.governance/evidence.yaml`、`.governance/ledger.yaml` 和 `.governance/rules.yaml` 维护项目事实。大型输出写入默认读取集之外，只登记有界 evidence ref。
...
已实施项目推荐通过 `.governance/agent-entry.md` 接续。任意 Agent 接手时必须先运行项目入口；如果同一项目存在多个并行需求，Agent 必须展示可接续需求并让人选择，不得从聊天历史或自然语言关键词猜测。若 Claude Code、Codex 或其他平台提示 `open-cowork` 不是可用 Skill，Agent 仍应把它当成项目内接手规则读取，而不是把人的请求判定为未知命令。
...
其他 participants、intent、step report、approve、review、closeout 等命令属于 Agent 内部执行面，不应作为人类操作教程展示。对人的汇报应聚焦“谁负责、要做什么、是否确认、当前步骤能否推进、证据在哪里”，并明确说明 gate、approval、review decision 和 acknowledgement。Human gate 的确认动作应尽量提供 `approve` / `revise` / `reject` 短选项，避免要求人输入长篇审批句。执行前必须明确提示协作者设置、Step 1-4 产出物状态，以及 review/lint 等外部规则是启用、暂停还是本轮明确跳过。
...
旧版本项目如果存在旧版大量治理目录，Agent 应通过安装、初始化、setup 或 onboard 入口自动检测、迁移并验证，不要求人额外理解或执行迁移命令。旧历史移动到 cold history 后只作为追溯材料，不进入默认读取集。清理 cold history 和卸载治理文件属于破坏性动作，仍必须先 dry-run、再显式确认并写入 receipt，不能把 dry-run 文案说成已经物理删除。
