...
变更：`v0.3.1-human-participation-runtime`
版本：`0.3.1`
...
本轮主题是 **人类参与运行时加固**。它承接 V0.3.0 的人类可见 Step 1-9、Step 5 / Step 8 / Step 9 硬门禁和收束可读性能力，针对团队真实实践反馈中暴露的运行时问题做收束强化。

本轮权威状态如下：

| 项目 | 结果 |
| --- | --- |
| 变更编号 | `v0.3.1-human-participation-runtime` |
| 归档 | 已完成 |
| review 决策 | approve |
| 最终一致性 | pass |
| Step 5 门禁 | `human-sponsor` 已批准 |
| Step 8 门禁 | `human-sponsor` 已批准 |
| Step 9 门禁 | `human-sponsor` 已批准 |
| 残留跟进项 | 无 |
...
1. **Step 1 意图接入可见性**
   ...
   - 在 prepare 前也能生成/查看 intent 与早期 step report。
...
5. **真实独立 review 调度证据**
   ...
   - reviewer mismatch bypass 被记录到 `human-gates.yaml`、`review.yaml` 和最终一致性摘要。

6. **归档后 idle 收束可读性**
   - 当前 `ocw status` 在 idle 状态下展示最近归档 closeout 摘要。
   - `ocw status --last-archive` 可直接查看最近归档、review 决策、最终一致性和 human gate 摘要。
...
输入为团队对 V0.3.0 的真实实践反馈：人仍然很难判断当前处于哪一步、prepare 是否等价于已完成、review 失败如何恢复、独立 review 是否真实发生。
...
方案采用增量式运行时加固，不推翻 V0.3.0 的 Step 1-9 框架。核心方向是：
...
- approval / lifecycle transition 后刷新已落盘的 step reports；
...
- step report 能聚合证据和 reviewer 运行证据。
...
- 接续建议：不作为 open-cowork V0.3.1 阻断；应作为本地个人域 Agent 环境维护事项继续跟进。
...
- 接续建议：已在本轮闭合；后续仍应保持真实 reviewer 调度证据。
...
- 接续建议：后续 Step 9 后应优先生成短 closeout，再扩展详细复盘，避免报告阶段再次被上下文拖垮。
...
- 判断：不构成本轮阻断，因为权威 archive receipt、Step 9 report、final consistency、maintenance status 和 `ocw status --last-archive` 均显示 Step 9 archived / final consistency pass。
- 风险：审计者若误读 `STATUS_SNAPSHOT.yaml` 为最终状态，会产生困惑。
- 建议：下一轮可考虑让归档时 status snapshot 明确标注 snapshot stage，或在 archive 后生成 final status snapshot。
...
当前残留跟进项：权威 archive receipt 中为 `[]`。上面的两个框架观察项不阻断 V0.3.1 close，只建议作为后续候选优化输入。
