...
这是一次 breaking-level 的运行时简化，但版本定义为 v0.3.11，不定义为 v4.0。原因是 open-cowork 仍处于正式大面积推广前的成熟阶段，本次目标是在 v0.3 系列内完成默认状态模型收敛。
...
open-cowork 必须支持多个团队成员和多个 Agent 协同推进项目，同时保持项目状态收敛。团队协作通过 participants、role bindings、gates、review decisions、external rules 和 carry-forward records 表达。团队规模增加不得导致每一步生成物数量成倍增加。
...
这些旧目录可以被迁移、prune或作为冷历史保留，但 v0.3.11 的正常命令不得再创建它们。
...
v0.3.11 的安装、初始化、setup 和 onboard 入口必须自动检测旧版大量治理目录、旧协议版本和迁移需求。人/团队的默认入口仍是自然语言交给 Agent；Agent 不应要求人额外理解或执行迁移命令。

v0.3.11 同时提供 Agent 内部使用的显式迁移命令，用于排障、dry-run 或受控恢复，例如：

```text
ocw migrate current-state
```
...
5. 把旧运行目录移动到 `.governance/cold/legacy/` 或标记为可prune。
6. 更新 `.gitignore`，避免旧 runtime/cache 进入普通上下文。
7. 生成迁移摘要，说明迁移了什么、保留了什么、建议prune什么。

迁移必须保留审计可追溯性，但不得把旧目录完整复制进新的 active model。

### 卸载与prune

v0.3.11 应支持prune功能，但必须安全、可预览、可确认。
...
ocw hygiene --prune --dry-run
ocw hygiene --prune --confirm
```
...
- prune完成后生成 prune receipt。

未来出现类似大机制变更时，默认安装实施入口必须沿用“自动识别 -> 自动升级/迁移 -> 验证 -> 失败即停止”的升级模式；prune和卸载仍沿用“dry-run -> 显式确认 -> receipt”的破坏性动作模式。
...
- `ocw init` / `ocw setup` / `ocw onboard`：创建或升级当前状态文件，自动迁移旧版大量治理目录并验证。
...
- `ocw migrate current-state`：显式把旧治理树转换为 v0.3.11 当前状态，主要供 Agent 排障、dry-run 或受控恢复使用。
- `ocw hygiene`：报告文件数量、大小、默认读取集大小、冷历史和prune候选。
- `ocw uninstall`：预览并执行安全卸载/prune。

旧命令可以在一个 release 内作为兼容 alias 保留，但正常文档和生成的 Agent 入口必须指向当前 round/rule/evidence 命令。旧命令如果需要创建 `changes/`、`archive/`、`runtime/`、`index/` 或 `local/`，必须失败并给出迁移指导，不能静默重建旧模型。
...
- `init/setup/onboard` 只创建或升级当前状态文件；遇到旧版大量治理目录时自动迁移、写 receipt 并验证，验证失败必须返回失败。
...
- `resume` 使用紧凑默认读取集。
- `migrate current-state` 能转换代表性旧项目且不丢失关键摘要。
- `uninstall --dry-run` 不删除文件，只输出计划。
- `hygiene --prune --dry-run` 能识别可prune旧产物。
...
文档必须说明 open-cowork 是一个 Agent-first collaboration governance protocol，并且 v0.3.11 默认收敛到 current-state layout。文档不得继续把 `.governance/changes/**` 和 `.governance/archive/**` 描述为正常操作模型。

## 发布验收标准

v0.3.11 满足以下条件才可以发布：

1. 新项目初始化为 v0.3.11 当前状态文件。
...
9. 旧版本识别、dry-run prune、迁移 receipt 和 prune receipt 可用。
10. 长周期压力测试通过。
11. v0.3.10 项目有清晰迁移路径。
12. 文档把 v0.3.11 描述为 open-cowork 默认当前形态，而不是并列模式。
13. open-cowork 仓库自身在发布前完成 v0.3.11 self-dogfood 迁移。
...
7. open-cowork 仓库必须在 v0.3.11 tag 前 dogfood 当前状态文件。
