...
- 如果 Agent 回复“找不到 open-cowork skill”，不要停止；应把 open-cowork 当成本项目的接手规则，直接读取 `.governance/agent-entry.md` 和 `.governance/current-state.md`。
...
Agent 必须把 open-cowork 当成项目级协作框架，而不是当前会话私有状态，也不是必须由平台预装的 Skill 名称。

## Agent 必须做什么

1. 先做项目级 activation / status。
2. 如果项目里有多个可接续需求，先确认或选择目标需求。
3. 只读取入口给出的 recommended read set。
4. 以当前 round 的 scope 作为执行边界。
5. 以 role bindings 作为 owner、reviewer 和 human gate 映射。
6. 用人能理解的状态面汇报，不展示命令清单。
7. 执行、验证、审查和 closeout 都必须写回项目事实。
