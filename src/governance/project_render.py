...
def render_current_state(state: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"- open_cowork_version: {state.get('protocol', {}).get('version', '')}",
        ...
        f"- participant_confirmation: {(active_round.get('participant_confirmation') or {}).get('status', 'pending')}",
        f"- external_rules_confirmation: {((active_round.get('external_rules') or {}).get('confirmation') or {}).get('status', 'pending')}",
