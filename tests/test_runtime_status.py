...
class RuntimeStatusTests(unittest.TestCase):
    ...
    def test_runtime_status_writes_machine_readable_status_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(steps_status["current_step"], 7)
            self.assertEqual(steps_status["next_step"], 8)
            self.assertEqual(steps_status["completed_steps"], [1, 2, 3, 4, 5, 6, 7])
            self.assertEqual(steps_status["blocked_steps"], [])
